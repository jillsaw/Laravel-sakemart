    ...
    <div class="row justify-content-center">
        <div class="col-md-10">
            ...
            <div class="row mb-4">
                <div class="col-md-6">
                    @if ($product->image)
                        <img src="{{ asset($product->image) }}" class="img-thumbnail sakemart-product-img-detail">
                    @else
                        <img src="{{ asset('img/dummy.png')}}" class="img-thumbnail sakemart-product-img-detail">
                    @endif
                </div>
                <div class="col">
                    <div>
                    ...
                    @auth
                        <form method="POST" action="{{route('carts.store')}}" class="align-items-end">
                            ...
                            <div class="form-group row mb-3 pt-2">
                                ...
                                <div class="col-sm-10">
                                    <input type="number" id="quantity" name="qty" min="1" value="1" class="form-control w-25 sakemart-form-parts">
                                </div>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col">
                                    @if(Auth::user()->favorite_products()->where('product_id', $product->id)->exists())
                                        <a href="{{ route('favorites.destroy', $product->id) }}" class="btn sakemart-favorite-button text-favorite w-100" onclick="event.preventDefault(); document.getElementById('favorites-destroy-form').submit();">
                                            <i class="fa fa-heart"></i>
                                            お気に入り解除
                                        </a>
                                    @else
                                        <a href="{{ route('favorites.store', $product->id) }}" class="btn sakemart-favorite-button text-favorite w-100" onclick="event.preventDefault(); document.getElementById('favorites-store-form').submit();">
                                            <i class="fa fa-heart"></i>
                                            お気に入り
                                        </a>
                                    @endif
            ...
            <div>
             <!--平均評価の可視化-->
             <span class="sakemart-star-rating" data-rate="{{ round($product->reviews->avg('score')*2)/2 }}">
                    ...
                    @auth
                        <form method="POST" action="{{ route('reviews.store') }}">
                            ...
                            <div class="mb-3">
                                ...
                                @enderror
                                <input type="text" class="form-control sakemart-form-parts" name="title">
                            </div>
                            <div class="mb-4">
                                ...
                                @enderror
                                <textarea name="content" class="form-control sakemart-form-parts" rows="4"></textarea>
                            </div>
                            <input type="hidden" name="product_id" value="{{$product->id}}">
                            <button type="submit" class="btn sakemart-submit-button text-white w-100">レビューを追加</button>
                        </form>
                    @endauth
                </div>
