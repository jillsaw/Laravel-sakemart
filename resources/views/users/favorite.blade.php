     ...
     @foreach ($favorite_products as $favorite_product)
       <div class="col-md-7 mt-2">
         ...
         <div class="container mt-3">
            <h5 class="w-100 sakemart-favorite-item-text">{{ $favorite_product->name }}</h5>
            <h6 class="w-100 sakemart-favorite-item-text">&yen;{{ $favorite_product->price }}</h6>
         </div>
         <div class="col-md-2 d-flex align-items-center justify-content-end">
            <a href="{{ route('favorites.destroy', $favorite_product->id) }}" class="sakemart-favorite-item-delete" onclick="event.preventDefault(); document.getElementById('favorites-destroy-form{{$favorite_product->id}}').submit();">
             削除
            </a>
        ...
        <div class="col-md-3 d-flex align-items-center justify-content-end">
          <form method="POST" action="{{ route('carts.store') }}" class="m-3 align-items-end">
            @csrf
              ...
              <button type="submit" class="btn sakemart-favorite-add-cart">カートに入れる</button>
          </form>
       </div>
     @endforeach
   </div>
